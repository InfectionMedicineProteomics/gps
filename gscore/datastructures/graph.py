...
class Graph:
    ...
    def add_edge(self, node_from, node_to, bidirectional=True):
        ...
        if node_from not in self._nodes:
            ...
            if node_from not in self._edges:
                ...
                self._edges[node_from] = []

            self._edges[node_from].append(
                node_to
            )

            if bidirectional:

                if node_to not in self._edges:

                    self._edges[node_to] = []

                self._edges[node_to].append(
                    node_from
                )
